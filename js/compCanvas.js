...
function drawDonut(ctx, cx, cy, value, maxVal, color) {
  ...
  ctx.closePath();
  ctx.fillStyle = RT.BG;
  ctx.shadowColor = RT.SD; ctx.shadowBlur = 6; ctx.shadowOffsetX = 3; ctx.shadowOffsetY = 3;
  ctx.fill();
  ctx.shadowColor = RT.SL; ctx.shadowBlur = 6; ctx.shadowOffsetX = -3; ctx.shadowOffsetY = -3;
  ctx.fill();
  ctx.restore();
...
export function renderCompCanvas({ headers, rows }, cfg) {
  ...
  const maxVal = (cfg.compMax && cfg.compMax > 0) ? cfg.compMax : Math.max(...items.map(d => d.value));
  ...
  items.forEach((d, i) => {
    ...
    ctx.fillStyle = RT.TC; ctx.textAlign = 'center'; ctx.textBaseline = 'top';
