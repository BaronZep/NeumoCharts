...
export function renderPieCanvas({ headers, rows }, cfg) {
  ...
  items.forEach((d, i) => {
    ...
    ctx.fillStyle = color;
    ctx.shadowColor = RT.SD; ctx.shadowBlur = 14; ctx.shadowOffsetX = 5; ctx.shadowOffsetY = 5;
    ctx.fill();
    ctx.strokeStyle = RT.BG; ctx.lineWidth = 4; ctx.shadowColor = 'transparent';
    ctx.stroke();
    ctx.restore();
    ...
    if (cfg.pieShowLabels) {
      ...
      ctx.save();
      ctx.strokeStyle = RT.TC; ctx.lineWidth = 1.5; ctx.globalAlpha = 0.6;
      ctx.beginPath(); ctx.moveTo(lx1, ly1); ctx.lineTo(lx2, ly2); ctx.lineTo(lx3, ly2); ctx.stroke();
      ctx.globalAlpha = 1; ctx.fillStyle = RT.TC;
  ...
  legendItems.forEach(it => {
    ...
    ctx.shadowColor = RT.SD; ctx.shadowBlur = 2; ctx.shadowOffsetX = 1; ctx.shadowOffsetY = 1;
    roundRect(ctx, lx, legendY - DOT / 2, DOT, DOT, 3); ctx.fill();
    ctx.restore();
    ctx.fillStyle = RT.TC; ctx.fillText(it.label, lx + DOT + DGAP, legendY);
    lx += it.w + IGAP;
  });
  ctx.restore();
